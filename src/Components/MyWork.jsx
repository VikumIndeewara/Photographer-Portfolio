  ...
  return (
    <div id="work" className="w-dvw h-full bg-black">
      <div className="container mx-auto lg:px-20 flex flex-col justify-center lg:max-w-6xl max-w-xl ">
        ...
        <div className="w-full h-full flex lg:justify-center my-3 overflow-hidden">
          {works.map((work, i) => (
            <button
              key={i}
              className={`${
                active == work ? "bg-brand1" : "bg-white"
              } text-black font-semibold py-3 px-5 lg:m-5 m-1  rounded-full hover:opacity-80`}
        ...
        <span className="text-white flex justify-end my-5 cursor-pointer hover:text-brand1">
          See More
        </span>
      </div>
    </div>
